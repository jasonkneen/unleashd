...
5. `d97e2064` -> not found in persisted sources searched (`~/.codex/sessions`, `~/.unleashd`, `~/.agent-viewer`).
...
1. Conversation runtime has both `id` and `sessionId` server-side ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):294-296).
2. Shared `ConversationSchema` exposes `id` only, not `sessionId` ([shared/src/index.ts](/Users/nicholasbardy/git/unleashd/shared/src/index.ts):292-336).
3. `toJSON()` serializes `id` but not `sessionId` ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):1077-1098).
...
Disk path conversion sets `id = session.sessionId` ([server/src/adapters/disk-adapter.ts](/Users/nicholasbardy/git/unleashd/server/src/adapters/disk-adapter.ts):86-109).

`hydrateConversation()` then uses that `id` as canonical conversation id ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):3701-3719).
...
`init` currently does `new Map(existing)` then overlays server conversations ([client/src/atoms/actions.ts](/Users/nicholasbardy/git/unleashd/client/src/atoms/actions.ts):309-314).
...
1. Codex: `~/.codex/sessions/YYYY/MM/DD/*.jsonl` ([server/src/adapters/registry.ts](/Users/nicholasbardy/git/unleashd/server/src/adapters/registry.ts):72-79).
2. Loader discovers/parses all adapters ([server/src/adapters/loader.ts](/Users/nicholasbardy/git/unleashd/server/src/adapters/loader.ts):42-89, 179-292).
3. Startup hydration and ongoing poll import these into runtime conversation map ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):3730-3785, 3895-4041).
...
Top-level filtering hides children only if parent ID is present in the current set ([client/src/components/Sidebar.tsx](/Users/nicholasbardy/git/unleashd/client/src/components/Sidebar.tsx):117-123).
...
`loadExistingConversations()` hardcodes `const limit = 500` ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):3734), even though env knobs exist (`CWV_STARTUP_INITIAL_LOAD_LIMIT` etc., [server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):118-121).
...
1. `CWV_TURN_IDLE_TIMEOUT_MS` ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):127)
2. `CWV_TURN_MAX_RUNTIME_MS` ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):128)
3. `CWV_TURN_TIMEOUT_KILL_GRACE_MS` ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):129)

Hot-reload SIGTERM drain behavior lives in signal handler block ([server/src/server.ts](/Users/nicholasbardy/git/unleashd/server/src/server.ts):3541-3642).

Validation previously run and passing:

1. `pnpm --filter @unleashd/server typecheck`
2. `pnpm test`
...
1. client-generated IDs at creation ([client/src/atoms/actions.ts](/Users/nicholasbardy/git/unleashd/client/src/atoms/actions.ts):175)
2. disk hydration keyed by provider session ID ([server/src/adapters/disk-adapter.ts](/Users/nicholasbardy/git/unleashd/server/src/adapters/disk-adapter.ts):93)
3. `init` merge retaining stale local map entries ([client/src/atoms/actions.ts](/Users/nicholasbardy/git/unleashd/client/src/atoms/actions.ts):309-314)
...
This server uses a local linked package ([server/package.json](/Users/nicholasbardy/git/unleashd/server/package.json):13):
